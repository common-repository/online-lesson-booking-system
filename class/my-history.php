...
class olb_logs extends olbHistory {
	...
	public static function line( $line, $r ) {
		...
		$increment = intval( $data['new'] ) - intval( $data['old'] );
		ob_start();
		echo '<tr>'."\n";
		...
		// Ver >= 0.4.1
			...
			if ( $increment == 0 ) {
				...
				if ( $data['old'] == $data['new'] ) {
					// extended
					if ( $data['days'] > 0 && ( isset( $data['oldterm'] ) && isset( $data['newterm'] ) ) && strcmp( $data['oldterm'], $data['newterm'] ) != 0 ) {
					...
					// deleted
					else if ( ( isset( $data['oldterm'] ) && isset( $data['newterm'] ) ) && strcmp( $data['oldterm'], $data['newterm'] ) != 0 && trim( $data['newterm'] ) == '' ) {
