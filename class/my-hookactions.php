...
<?php
	...
	public static function update_log( $result ) {
		...
		if ( ( $result['old'] != $result['new'] ) || ( isset( $result['oldterm'] ) && isset( $result['newterm'] ) && ( $result['oldterm'] != $result['newterm'] ) ) ) {
			$prefix = $wpdb->prefix.OLBsystem::TABLEPREFIX;
			$table = $prefix."logs";
			$increment = intval( $result['new'] ) - intval( $result['old'] );
			$now = current_time('timestamp');
			$ret = $wpdb->insert(
				$table,
				array(
		...
		if ( $user->isLoggedIn() && $user->isAdmin() ) {
			...
			if ( isset( $olb->qs['room_id'] ) ) {
				...
				if ( !empty( $target_user->data['id'] ) && $target_user->isRoomManager() ) {
					...
					$olb->admin_pretend = $_SESSION['admin_pretend'];
				}
			}
			// user
			elseif ( isset( $olb->qs['user_id'] ) ) {
...
EOD;
	...
	public static function activation() {
		...
		$version = ( get_option('olbversion') ) ? get_option('olbversion') : array();
