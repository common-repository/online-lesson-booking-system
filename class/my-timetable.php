...
class olbTimetable extends OLBsystem{
	 ...
	 */
	public static function canCancellation( $result, $room_id, $date, $time ){
		global $olb;
		...
		elseif( olbTimetable::isTimeover('cancel', $date, $time ) ){
			$result['code'] = 'CANCEL_TIMEOVER';
	...
	public static function canReport( $room_id, $date, $time ){
		...
		$result = array();
		$result = apply_filters( 'olb_can_cancellation', $result, $room_id, $date, $time );
...
EOD;
	...
	public static function htmlCancelForm($out = false){
		...
		if(!$error) {
			...
			$result = array();
			$result = apply_filters( 'olb_can_cancellation', $result, $olb->room_id, $date, $time );
			/** 
			 *	$result = array( 
			 *		'code'   => 'RESERVE_OK',
			 *		'record' => array(
